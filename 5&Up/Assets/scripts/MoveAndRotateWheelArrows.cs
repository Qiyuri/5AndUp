...
public class MoveAndRotateWheelArrows : MonoBehaviour
{
    [Header("Wielen")]
    ...
    void Update()
    {
        ...
        // Verticale beweging (vooruit/achteruit)
        ...
        if (wheel1Grounded)
        {
            if (Input.GetKey(KeyCode.LeftArrow))
                targetHorizontalSpeed = -maxSpeed;
        ...
        currentHorizontalSpeed = Mathf.MoveTowards(currentHorizontalSpeed, targetHorizontalSpeed, accel * Time.deltaTime);
        ...
        // Horizontale beweging (links/rechts)
        if (Mathf.Abs(currentHorizontalSpeed) > 0.001f)
        {
            Vector3 horizontalMovement = Vector3.right * currentHorizontalSpeed * Time.deltaTime;
            transform.Translate(horizontalMovement, Space.Self);

            float horizontalRotation = rotationSpeed * Mathf.Sign(currentHorizontalSpeed) * Time.deltaTime;
            if (wheel1 != null)
                wheel1.Rotate(0f, 0f, -horizontalRotation, Space.Self);
            if (wheel2 != null)
                wheel2.Rotate(0f, 0f, -horizontalRotation, Space.Self);
        }
